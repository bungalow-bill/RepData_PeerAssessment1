...
The following graph shows a histogram of the total number of steps taken each day of the `r max(activity.dat$Date)-min(activity.dat$Date)+1` days in the dataset.     
...
Mean number of steps per day:  `r format(step.mean)`    
Median number of steps per day:  `r step.median`     
...
```{r daily_pattern, fig.width=8,fig.height=6}
...
As illustrated by the dashed red line, the time interval that had the highest average number of steps was `r format(max.int[1,1],"%H:%M")` which had an average number of steps equal to `r max.int[1,2]`       
...
```{r missing_data, fig.height=6,fig.width=8}
...
There appear to be `r missing.count` rows with missing data, which occur on `r length(missing.dates)` separate dates.  checking the data for those dates we find that they are missing data for all intervals in the day.  Using the previously calculated average values for each interval, we can _fill in_ the missing values according to the interval in which they occur.  Using this approach, the histogram of total steps per day, shown below, differs from the original in that the bin containing the mean indicates an additional `r length(missing.dates)` days.    
```{r fill_data, fig.height=6,fig.width=8}
...
----------|----------|--------
Mean | `r format(step.mean)` | `r format(filled.mean)`
Median | `r step.median` | `r filled.median`
...
```{r filled_daily_pattern, fig.width=8,fig.height=6}
...
```{r filled_calendar, fig.height=6,fig.width=8}
...
```{r weekends, fig.height=6,fig.width=8}
